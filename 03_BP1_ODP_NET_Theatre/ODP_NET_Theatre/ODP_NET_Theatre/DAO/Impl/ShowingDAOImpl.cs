...
namespace ODP_NET_Theatre.DAO.Impl
{
    public class ShowingDAOImpl : IShowingDAO
    {
        ...
        {
            String insertSql = "insert into showing (date_sh, time_sh, numofspec_sh, play_id_pl, scene_id_sc, ordnum_sh) " +
                "values (:date_sh, :time_sh, :numofspec_sh, :play_id_pl, :scene_id_sc, :ordnum_sh)";
            String updateSql = "update showing set date_sh=:date_sh, time_sh = :time_sh, numofspec_sh = :numofspec_sh, play_id_pl = :play_id_pl, " +
        ...
        {
            string query = "select play_id_pl, sum(numofspec_sh) " +
        ...
        public List<PlayShowingsStatsDTO> FindSumAvgCountForShowingPlay()
        {
            ...
            string query = "SELECT play_id_pl, SUM(numofspec_sh), AVG(numofspec_sh) , COUNT(*) " +
                "FROM Showing " +
                "GROUP BY play_id_pl";
        ...
        {
            string query = "select name_sc, numofseats_sc, theatre_id_th, sc.id_sc, ordnum_sh, date_sh, time_sh, numofspec_sh, play_id_pl " +
            ...
            using (IDbCommand command = connection.CreateCommand())
            {
                ...
                using (IDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ShowingDeleteDTO showing = new ShowingDeleteDTO(reader.GetString(0),
                                reader.GetInt32(1), reader.GetInt32(2), reader.GetInt32(3), reader.GetInt32(4),
        ...
        {
            string query = "select ordnum_sh, date_sh, time_sh, numofspec_sh, play_id_pl, scene_id_sc " +
                "from showing " +
